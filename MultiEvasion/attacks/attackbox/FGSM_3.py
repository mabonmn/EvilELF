"""
FGSM for models with raw bytes input
target on 3 detectors
"""
import torch
...
class FGSM(Attack):
    ...
    def __init__(self,
        ...
        ## 继承父类Attack的参数
        super(FGSM, self).__init__(model_1=model_1, model_2=model_2,model_3=model_3)
        ## 不同方式继承父类
    ...
    def perturbation(self,
        ...
        if len(index_to_perturb) > 0:
            ...
            if self.model_3:
                (adv_x_preds_1, adv_y_1), (adv_x_preds_2, adv_y_2),(adv_x_preds_3,adv_y_3) = \
                    self._get_print_return_results(inputs, model_1=self.model_1, model_2=self.model_2,model_3=self.model_3, test_x_init=True)
                pert_size = len(self.index_to_perturb)
                return (adv_x_preds_1, adv_y_1), (adv_x_preds_2, adv_y_2),(adv_x_preds_3,adv_y_3), pert_size, num_evasion_by_benign_content, \
        ...
        if self.model_3:
            (adv_x_preds_1, adv_y_1), (adv_x_preds_2, adv_y_2),(adv_x_preds_3,adv_y_3) \
                = self._get_print_return_results(x_adv_init,
        ...
        adv_x1 = self._get_advX(x=x_adv_init,
                                embed_x=embed_x_1,
        ...
        if self.model_3:
            ...
            ## get adv_x by applied with FGSM based perturbations
            ## by add pert on adv_x initiated with benign contents or random bytes
            adv_x3 = self._get_advX(x=x_adv_init,
                                    embed_x=embed_x_3,
        ...
        if self.model_2 and not self.model_3:
            ...
            ## three models
            (adv_x_preds_1, adv_y_1), (adv_x_preds_2, adv_y_2),(adv_x_preds_3,adv_y_3) = \
                self._get_print_return_results(adv_x, model_1=self.model_1, model_2=self.model_2,model_3=self.model_3, test_x_init=False)
            pert_size = len(self.index_to_perturb)
            return (adv_x_preds_1, adv_y_1), (adv_x_preds_2, adv_y_2),(adv_x_preds_3,adv_y_3), pert_size, num_evasion_by_benign_content, \
                   num_index_change_in_total_group, num_index_change_by_gradient_group
        elif not self.model_2 and not self.model_3:
