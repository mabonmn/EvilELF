"""
FGSM for models with different input format.
one with raw byte input: model 1
another with image input: model 2
"""
import torch
...
class FGSM(Attack_different_input):
    """
    ...
    def __init__(self,
        ...
        ## 继承父类Attack的参数
        super(FGSM, self).__init__(model_1=model_1,
                                   model_2=model_2,
                                   height=height,
                                   width=width)
        ## 不同方式继承父类
        #Attack_different_input.__init__(model_1, model_2)
    ...
    def perturbation(self,
        ...
        if self.pert_init_with_benign:
            benign_contents = self.get_section_content_from_folder(folder_path=None,
                num_sections=100, target_section='.text')
        else:
        ...
        if not self.model_2:
            ...
            x_adv_2 = torch.reshape(x_adv, (1, 1, self.height, self.width)) / 255
            preds_2, embed_x_2 = self._forward(x_adv_2, self.model_2,model_type='image')
        ...
        if self.model_2:
            ## model 2: image based
            grad_sign_for_perturb_2 = torch.autograd.grad(loss, x_adv_2,retain_graph=False, create_graph=False)[0].sign()

            ## get adv_x by applied with FGSM based perturbations
            ## by add pert on adv_x initiated with benign contents or random bytes
            adv_x2 = self._get_advImage(x=x_adv_2,
