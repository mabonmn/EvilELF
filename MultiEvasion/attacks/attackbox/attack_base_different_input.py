...
class Attack_different_input(object):
    """
    ...
    def __init__(self,
        ...
        model 1: byte based model, model 2: image based model
        Arguments:
    ...
    def _forward(self, input, model,model_type='byte'):
        ...
        """
        if model_type == 'byte':
            ## byte input --> embed layer --> embed first
            embed = model.embed
            embed_x = embed(input.long()).detach()
        else:
            ## image input --> no embed layer
            embed_x = input
        if embed_x.requires_grad == False:
            embed_x.requires_grad = True    # Ture --> 加入到反向传播中计算梯度
        output = model(embed_x.to(self.device).float())
    ...
    def _get_preds_label(self,input:list,model_1=None,model_2=None):
        """
        get the label for the input toward models

        input: list or torch.tensor
        model_1: pytorch model  (raw byte)
        model_2: pytorch model (image)
        ...
        if model_2:
            ...
            X2 = torch.reshape(X,(1,1,self.height,self.width)) / 255
            preds_score_2, _ = self._forward(X2, model_2,model_type='image')
            y_2 = torch.argmax(preds_score_2, 1).item()
            # y_2 = np.argmax(preds_score_2)
    ...
    def _get_print_return_results(self,input:list,model_1=None,model_2=None,test_x_init=False):
        """
    ...
    def _get_final_grad_sign(self,grad_sign_1,grad_sign_2,overshot):
        """
    ...
    def _get_advImage(self,x=None,sign=None,eps=None,index_to_perturb=None):
        """
        for image based models
        obtain adversarial malware by add pert on input: operate based on the image shape e.g. [1,1,320,320] (batch,c,h,w),
        where x is normalized to [0,1] in ahead
        sign (shape[1,1,320,320]) --> pert=eps*sign--> normalized adv_x (shape=[1,1,320,320]) = x + (-pert)
        --> reverse normalized adv_x to [0,255]

        Note: perturbation is based on gradient descent direction, which is the direction to minimize the loss
              goal here is to maximize loss to make the prediction to other label (instead original label)
              should add gradient in its increase direction (i.e., -gradient_sign*eps)

        :param x: original input binary file, already normalized to [0,1] (shape=[batch,c,h,w])
        :param sign: grad sign of model (shape=[batch,first_n_bytes,embed_size])
        :param eps:-->float: weight for perturbation
        :param index_to_perturb: index position to perturb pe file without breaking functionality
        :return: -->ndarray: adversarial/crafted x (shape=[first_n_bytes] 1-d)
        """
        ## flatten input
        x_flatten = x.squeeze().flatten().detach().cpu().numpy()
        x_adv_norm = copy.deepcopy(x_flatten)

        ## get pert by multiple eps. (shape[batch=1,c,h,w])
        pert = eps * sign   # (0,1)
        pert_flatten = pert.squeeze().flatten().detach().cpu().numpy()
        ...
        for i,p in enumerate(index_to_perturb):
            ...
            ## goal here is to maxmize loss to make the prediction to other label
            ## should add gradient increase direction (-gradient_sign)
            x_adv_norm[p] = x_adv_norm[p] + (-pert_flatten[p])

        ## reverse normalized adv_x_norm_in_perturb_index to pre-norm.
        x_adv_norm = np.clip(x_adv_norm,0,1)
        x_adv = x_adv_norm * 255

        ## clamp to [0, 255]
        adv_x = np.clip(x_adv, self.x_box_min, self.x_box_max)
        adv_x = adv_x.astype(int)

        return adv_x


    def _get_advImage_for_FFGSM(self,x=None,sign=None,eps=None,index_to_perturb=None,alpha=10/255):
        """
        for image based models
        obtain adversarial malware by add pert on input: operate based on the image shape e.g. [1,1,320,320] (batch,c,h,w),
        where x is normalized to [0,1] in ahead
        ...
        ## reverse normalized adv_x_norm_in_perturb_index to pre-norm.
        delta = np.clip(x_adv_norm - x_flatten,-alpha,alpha)
        x_adv = np.clip(x_flatten + delta,0,1) * 255
        adv_x = x_adv.astype(int)
    ...
    def _get_advX_for_PGD(self,
        ...
        ## get pert by multiple eps. (shape[batch=1,len(index_to_perturb),embed_size])
        pert = eps * sign   # (0,1)
        ...
        for i,p in enumerate(index_to_perturb):
            ...
            ## values from original input (without perturbation) in the index from index_to_perturb
            embed_x_init_norm_in_pertrub_index[i,:] = embed_x_init_norm[p]
            ## values from adv_x (after perturbation) in the index from index_to_perturb
    ...
    def _get_advImage_for_PGD(self,
                              x=None,
                              x_adv=None,
                              sign=None,
                              eps=0.1,
                              ...
                              index_to_perturb=None):
        """
        obtain adversarial malware (image format) by add pert on input

        sign ( shape[1,1，320,320]) --> pert=eps*sign
        --> adv_x (shape=[102400]) = x + (-pert)
        --> clip accumulate normalized perturbation inside [-alpha,alpha], clip(accumulate_pert_norm,-eps,eps),
        where:accumulate_pert_norm = adv_x - x_original, x_original: original input after normalized to [0,1]
        --> reverse normalized adv_x -->  adv_x (-->shape[102400] 1-d in range [0,255])

        Note: perturbation is based on gradient descent direction, which is the direction to minimize the loss
              goal here is to maximize loss to make the prediction to other label (instead original label)
              should add gradient in its increase direction (i.e., -gradient_sign*eps)

        :param x: original input binary file，already normalized to [0,1] (with benign content or random values initiation)
                (shape=[batch,c,h,w])
        :param x_adv: adv_x in the last pgd iteration (shape=[1,1,320,320], already normalized)
        :param sign: grad sign of model (shape=[batch,c,h,w])
        :param alpha:-->float: maximum perturbation. (default:0.7)
        :param eps:-->float: perturbation step size (default:0.07)
       :return: -->ndarray: adversarial x (shape=[first_n_bytes] 1-d)
        """
        ## x: to ndarray, flatten; [1,1,320,320] --> [102400]
        x_init_flatten = x.squeeze().flatten().detach().cpu().numpy()
        x_init_norm = copy.deepcopy(x_init_flatten)

        ## x_adv: shape[1,1,320,320] -->[102400]
        x_adv_flatten = x_adv.squeeze().flatten().detach().cpu().numpy()
        x_adv_norm = copy.deepcopy(x_adv_flatten)

        ## get pert by multiple eps. (shape[1,1,320,320] --> [102400])
        pert = eps * sign   # (0,1)
        pert_flatten = pert.squeeze().flatten().detach().cpu().numpy()

        ## get adv_x_norm_in_perturb_index by adding pert on input based on pertubation index.
        ## i.e. final value after applied perturbations in the position of perturb index
        ## --> (shape[length of index_to_perturb])
        adv_x_norm_in_pertrub_index = np.zeros(np.array(index_to_perturb).shape)
        x_init_norm_in_pertrub_index = np.zeros(np.array(index_to_perturb).shape)
        for i,p in enumerate(index_to_perturb):
            ## perturbation is based on gradient desecent, which is the direction to minimize the loss
            ## goal here is to maxmize loss to make the prediction to other label
            ## should add gradient increase direction (-gradient_sign)
            ...
            ## values from original input (without perturbation) in the index from index_to_perturb
            x_init_norm_in_pertrub_index[i] = x_init_norm[p]
            ## values from adv_x (after perturbation) in the index from index_to_perturb
            adv_x_norm_in_pertrub_index[i] = x_adv_norm[p] + (-pert_flatten[p])

        ## limit accumulated perturbation to the range [-alpha,alpha] in each iteration
        accumulate_pert = np.clip(adv_x_norm_in_pertrub_index - x_init_norm_in_pertrub_index, -alpha, alpha)
        ## get the final adv_x in the index of index_to_perturb, clip to [0,1]
        adv_x_norm_in_perturb_index = np.clip(x_init_norm_in_pertrub_index + accumulate_pert, 0, 1)

        ## get adv_x by updating final value in perturb index to orginal input x
        adv_x = self._update_input(input=x_init_norm,index_to_perturb=index_to_perturb,pert=adv_x_norm_in_perturb_index)
        ## convert from [0,1] to [0,255]
        adv_x = (adv_x * 255).astype(int)
        ## clamp to [0, 255]
        adv_x = np.clip(adv_x, self.x_box_min, self.x_box_max)
    ...
    def _final_adv_x_selection(self, x=None, adv_x1=None, adv_x2=None, verbose=True, index_to_perturb=None):
        """
