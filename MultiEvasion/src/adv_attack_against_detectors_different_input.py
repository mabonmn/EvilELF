"""
produce generic adversarial malware that evade multiple malware detectors with different input format,
one is based on raw byte, another one is based on image
...
from src.model import MalConv_freezeEmbed, ResNet18
import numpy as np

from attacks.attackbox import FGSM_different_input, PGD_different_input, FFGSM_different_input
...
class adv_evasion_attack_sync(Attack_different_input):
    """
    model 1: raw byte based; model 2: image based
    ...
    """
    def __init__(self,
                 model1=None,   ## raw byte
                 model2=None,   ## image
                 log_file_result_name=None):

        ## 继承父类
        Attack_different_input.__init__(self,model1,model2)
    ...
    def get_predicted_malware(self,
        ...
        for i,batch in enumerate(data_iterator):
            ...
            if model_2: ## image
                ## reshape to image format [batch,channel,h,w] and normalized to [0,1]
                X2 = torch.reshape(X,(1,1,args.height,args.width)) / 255
                confidence_2,_ = forward_prediction._forward(X2, model_2,model_type='image')
    ...
    def test_loop(self,
                  ...
                  verbose:bool=True):
        """
        model_1: byte based model; model_2: image based model
...
def main_test():
    ...
    model_2 = ResNet18(1,2).to(device)
    ...
    if args.adversary == 'FGSM':
        adversary = FGSM_different_input.FGSM(model_1=model_1,
                                              model_2=model_2,
                                              eps=args.eps,
                                              w_1=0.5,
                                              w_2=0.5,
        ...
        adversary = FFGSM_different_input.FFGSM(model_1=model_1,
                                                model_2=model_2,
                                                eps=args.eps,
                                                alpha=args.alpha,
                                                w_1=0.5,
        ...
        adversary = PGD_different_input.PGD(model_1=model_1,
                                            model_2=model_2,
                                            alpha=args.alpha,
                                            eps=args.eps,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--height', default=320,type=int,help='image height')
