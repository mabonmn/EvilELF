"""
produce generic adversarial malware that evade multiple malware detectors (up to 3),
...
from src.util import ExeDataset, get_acc_FP,get_acc_FP_3, get_adv_name
from src.model import MalConv_freezeEmbed,FireEye_freezeEmbed,AvastNet_freezeEmbed
import numpy as np

from attacks.attackbox import FGSM,PGD,FFGSM
from attacks.attackbox import FGSM_3,PGD_3
...
class adv_evasion_attack_sync(Attack):
    """
    perform adversarial evasion attack against multiple (e.g., three) malware detectors at same time.
    ...
    def __init__(self,
        ...
        ## 继承父类
        Attack.__init__(self,model_1=model1,model_2=model2,model_3=model3)
    ...
    def get_predicted_malware(self,
        ...
        three models: discard samples that assigned label with benign by all models, otherwise keep for attacking
        ...
        for i,batch in enumerate(data_iterator):
            ...
            if model_3:
                ...
                confidence_3, _ = forward_prediction._forward(X, model_3)
                y_pred_3.append(np.argmax(confidence_3))
                if confidence_1[0] > 0.5 and confidence_2[0] > 0.5 and confidence_3[0] > 0.5:
                    continue
                else:
        ...
        if not model_3:
            print(f'Model1 {args.model_path_1.split("/")[-1].split(".")[0]} Acc: {accuracy_score(y_pred_1, all_y)}; Model2 {args.model_path_2.split("/")[-1].split(".")[0]} Acc: {accuracy_score(y_pred_2, all_y)}')
        else:
            print(
                f'Model1 {args.model_path_1.split("/")[-1].split(".")[0]} Acc: {accuracy_score(y_pred_1, all_y)}; Model2 {args.model_path_3.split("/")[-1].split(".")[0]} Acc: {accuracy_score(y_pred_2, all_y)};Model3 {args.model_path_3.split("/")[-1].split(".")[0]} Acc: {accuracy_score(y_pred_3, all_y)}')
        if verbose:
            with open(self.log_file_for_result, 'a') as f:
                ...
                if not model_3:
                    ...
                    print(f'Model1 {args.model_path_1.split("/")[-1].split(".")[0]} Acc: {accuracy_score(y_pred_1,all_y)}; Model2 {args.model_path_2.split("/")[-1].split(".")[0]} Acc: {accuracy_score(y_pred_2,all_y)}; Model3 {args.model_path_3.split("/")[-1].split(".")[0]} Acc: {accuracy_score(y_pred_3,all_y)}',file=f)
                print('\n',file=f)
    ...
    def test_loop(self,
        ...
        for i,(X,y) in enumerate(zip(X_mal,y_mal)):
            ...
            if model_2 and model_3:
                ## three models
                (adv_x_preds_1, adv_y_1), (adv_x_preds_2, adv_y_2),(adv_x_preds_3,adv_y_3),\
                pert_size, num_evasion_by_benign_content, num_index_change_in_total_group, \
                ...
                adv_labels.append((adv_y_1, adv_y_2,adv_y_3))
            if model_2 and not model_3:
                ## tow models
            ...
            elif not model_2 and not model_3:
                ## one models
        ...
        print(f'all labels {adv_labels}')

        single_model = (False if model_2 else True)
        if self.model_3:
            ## three models
            acc, FP, (acc1, FP1), (acc2, FP2),(acc3,FP3) = get_acc_FP_3(adv_labels, single_model=single_model)
        elif not self.model_3:
            ## one/two models
        ...
        if self.model_3:
            print(f'Three Models, Acc against adv_x (at least one detector predicted as malware): {acc}')
        else:
            print(f'Two Models: {single_model}, Acc against adv_x (at least one detector predicted as malware): {acc}')
        ...
        print(f'Model1 MalConv: {acc1} accuracy for detecting adversarial malware. FP: {FP1} number of adv_x evade successfully.')
        print(f'Model2 FireEye: {acc2} accuracy for detecting adversarial malware. FP: {FP2} number of adv_x evade successfully.')
        if self.model_3:
            print(f'Model3 AvastNet: {acc3} accuracy for detecting adversarial malware. FP: {FP3} number of adv_x evade successfully.')
        ...
        if verbose:
            with open(self.log_file_for_result, 'a') as f:
                ...
                if self.model_3:
                    print(f'Three Models, Acc against adv_x (at least one detector predicted as malware): {acc}',file=f)
                else:
                ...
                print(f'Model1 MalConv: {acc1} accuracy for detecting adversarial malware. FP: {FP1} number of adv_x evade successfully.',file=f)
                print(f'Model2 FireEye: {acc2} accuracy for detecting adversarial malware. FP: {FP2} number of adv_x evade successfully.',file=f)
                if self.model_3:
                    print(f'Model3 AvastNet: {acc3} accuracy for detecting adversarial malware. FP: {FP3} number of adv_x evade successfully.',file=f)
...
def main_test():
    ...
    if args.model_path_3:
        model_3 = AvastNet_freezeEmbed(vocab_size=256).to(device)
        model_3.load_state_dict(torch.load(args.model_path_3,map_location=device))
        model_3.eval()
    else:
        model_3 = None
    ...
    if args.adversary == 'FGSM':
        if model_3:
            adversary = FGSM_3.FGSM(model_1=model_1,
                                    model_2=model_2,
                                    model_3=model_3,
                                    eps=args.eps,
                                    w_1=args.w_1,
        ...
        if model_3:
            adversary = PGD_3.PGD(model_1=model_1,
                                  model_2=model_2,
                                  model_3=model_3,
                                  alpha=args.alpha,
                                  eps=args.eps,
    ...
    return model_1,model_2,model_3,adversary,test_data_loader
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_path_3', default=None, help='path for trained model 3')
    parser.add_argument('--w_1', default=0.33, type=float, help='loss weight of model 1')
    parser.add_argument('--w_2', default=0.33, type=float, help='loss weight of model 2')
    parser.add_argument('--w_3', default=0.33, type=float, help='loss weight of model 3')
    ...
    if args.adversary == 'FGSM':
        file_name = ('_' + args.adversary +'_eps'+str(args.eps) + '_extend'+str(args.preferable_extension_amount)
                     +'_shift'+str(args.preferable_shift_amount)+ '_' + evasion_attack_name+ '_w1_'+str(args.w_1)+
                     '_w2_'+str(args.w_2)+ '_w3_'+str(args.w_3) + '.').join(file_name_split[-1].split('.'))
    elif args.adversary == 'FFGSM':
    ...
    model_1, model_2, model_3, adversary, test_data_loader = main_test()
